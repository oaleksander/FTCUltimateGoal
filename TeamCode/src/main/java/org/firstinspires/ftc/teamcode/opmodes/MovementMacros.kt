...
object MovementMacros {
     ...
     fun avoidRingStack() {
          movement.pos(Pose2D(odometry.robotCoordinates.x + 15 * sideSign, -60.0, Double.NaN),
                       distanceTolerance = 10.0, angularTolerance = .5)
     }
     ...
     fun moveWobble() {
          ...
          movement.followPath(MotionTask(wobblePose.minus(Vector2D(0.0, WobblePlacementOffset.radius()).rotatedCW(error.acot())),
                              ...
                              distanceTolerance =  3.0, angularTolerance = toRadians(3.0))
     ...
     fun pickSecondWobble() {
          ...
          movement.followPath(MotionTask(wobblePose.minus(Vector2D(0.0, WobblePlacementOffset.radius()).rotatedCW(error.acot())),
                                         error.acot() - WobblePlacementOffset.acot()),
                              distanceTolerance = 1.5, angularTolerance =  toRadians(1.0))
          while (movement.pathFollowerIsActive() && opMode.opModeIsActive()) {
     ...
     private fun powerShotShootingPose(powerShot: PowerShot): Pose2D {
          val heading = Range.clip(movement.getError(Pose2D(powerShotPose(PowerShot.MEDIUM), Double.NaN)).acot(),
                                   toRadians(-17.0), toRadians(17.0))
          val shootingCoordinates = powerShotPose(PowerShot.MEDIUM).minus(
               Vector2D(0.0, PowerShotShootingDistance).rotatedCW(heading))
          return when (powerShot) {
               PowerShot.LEFT -> Pose2D(shootingCoordinates,
                                        heading + toRadians(PowerShotShootingAngle) + powerShotPose(PowerShot.LEFT).minus(
                                             shootingCoordinates).acot() - powerShotPose(PowerShot.MEDIUM).minus(
                                             shootingCoordinates).acot())
     ...
     */
     ...
     fun pickupRings(ultimate: Boolean = false) {
          val heading = movement.getError(Pose2D(ringStackPose, Double.NaN)).acot()
          when (openCVNode.stackSize) {
               StackSize.FOUR -> {
                    movement.pos(
                         Pose2D(ringStackPose - Vector2D(0.0, RingStackApproachOffset).rotatedCW(heading), heading + Math.PI),
                         distanceTolerance = 5.0, angularTolerance = toRadians(5.0))
                    conveyor.enableConveyor = true
                    movement.pos(
                         Pose2D(ringStackPose - Vector2D(0.0, RingStackFirstRingOffset).rotatedCW(heading), heading + Math.PI),
                    ...
                    //conveyor.enableConveyor(true)
                    movement.pos(
                         Pose2D(ringStackPose - Vector2D(0.0, RingStackFourthRingOffset).rotatedCW(heading), heading + Math.PI),
